    ...
    public class SQLGetHistoryProduct
    {
        ...
        public DataTable GetHistory(string s_Id)
        {
            ...
            DataTable data = new DataTable("ARTYKUL");
                    ...
                    string s_Operation = "SELECT A.NUMER, A.KOREKTA, convert(datetime, A.DATA - 36163) as DATA, A.KONTRAHENT, " +
                                        "C.NUMER, convert(datetime, C.DATA_SPRZEDAZY - 36163, 103) AS DATA_SPRZEDAZY, " +
                                        "A.ILOSC, " +
                                        "A.RODZAJ, A.POZOSTALO, A.CENA, A.ZYSK " +
                                        "FROM  { oj HISTORIA_MAGAZYNU A LEFT OUTER JOIN DOKUMENT_HANDLOWY_V C " +
                                        "ON A.ID_DOK_HANDLOWEGO = C.ID_DOKUMENTU_HANDLOWEGO } , " +
